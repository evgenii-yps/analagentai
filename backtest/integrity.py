...
async def check_continuity(
    ...
    *,
    ...
    ``bar`` ОБЯЗАТЕЛЕН К ПЕРЕДАЧЕ и значения по умолчанию не имеет — ни здесь,
    ни где-либо ещё в чтении ``backtest.candles``. До Замера 0 здесь стояло
    ``bar: str = "1H"``, и это было безобидно ровно до того дня, когда в той же
    таблице появились дневные, недельные и месячные бары: с этого момента
    вызов, забывший про масштаб, молча получал бы отчёт о непрерывности
    ЧАСОВОГО ряда на вопрос про дневной. Пропуск в дневном ряде выглядел бы
    как его отсутствие — то есть проверка врала бы в сторону «всё хорошо».

    ``None`` допустим ТОЛЬКО для funding: у ставок финансирования масштаба не
    существует. Тип говорит об этом прямо, а отсутствие значения по умолчанию
    заставляет вызывающего написать ``bar=None`` осознанно, а не промолчать.
    """
    if series == SERIES_CANDLES and bar is None:
        raise ValueError(
            "check_continuity: для ряда свечей масштаб обязателен. bar=None "
            "допустим только для funding. Без масштаба запрос смешал бы часовые "
            "бары со старшими, которые лежат в той же таблице с Замера 0"
        )
