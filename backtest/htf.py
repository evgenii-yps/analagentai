...
# --- Допуск сравнения O/H/L/C (§7 ТЗ; пересмотрен Замером 0.1) -------------
#
# ОБЕ КОНСТАНТЫ ЖИВУТ ЗДЕСЬ И ТОЛЬКО ЗДЕСЬ. По месту сравнения число не
# пишется никогда: допуск, размноженный по файлам, однажды разойдётся сам с
# собой и разойдётся молча.
#
# ОТКУДА ВЗЯТЫ ЗНАЧЕНИЯ — ИЗ ИЗМЕРЕНИЯ, А НЕ ИЗ СООБРАЖЕНИЙ УДОБСТВА. Прогон на
# боевых данных 11.09.2026 с АБСОЛЮТНЫМ допуском 1e-8 дал 513 расхождений на
# 10 759 сравнений, и почти ни одно не было ошибкой сборки. На ранних участках
# рядов OKX отдаёт дневные бары с ОБРЕЗАННЫМ (именно обрезанным, не
# округлённым) третьим знаком: SOL 44,789 -> 44,78; 49,129 -> 49,12;
# DOGE 0,141581 -> 0,14158. В часовых барах третий знак присутствует.
#
# Одна абсолютная мерка была приложена к величинам, различающимся в миллион
# раз. При цене BTC в десятки тысяч допуск 1e-8 недостижим в принципе и не
# сработал ни разу; при цене DOGE в 0,0002 он избыточен на порядки и дал 360
# срабатываний — при ОДИНАКОВОМ качестве данных:
#
#   инструмент | не сошлось | доля  | макс. относительное
#   BTC-USDT   |     0      | 0%    | —
#   ETH-USDT   |     1      | 0,01% | 1,17e-03
#   XRP-USDT   |     1      | 0,01% | 2,25e-04
#   SOL-USDT   |   413      | 6,13% | 2,01e-04
#   DOGE-USDT  |   360      | 5,35% | 1,63e-03
#
# ИЗМЕРЕННОЕ РАСПРЕДЕЛЕНИЕ по 775 разошедшимся полям (11.09.2026):
#   до 1e-6 — 64 (8%); до 1e-5 — 656 (85%); до 1e-4 — 53 (7%); свыше 1e-3 — 2.
#
# 5e-4 (0,05%) закрывает все 773 случая огрубления и ОСТАВЛЯЕТ ВИДИМЫМИ оба
# случая свыше 1e-3 — настоящее рассогласование внутри данных самой OKX за
# 18.12.2022 (ETH: часовой максимум 1196,00 против дневного 1194,60; то же у
# DOGE и XRP). От типичного расхождения 5e-5 это десятикратный запас.
#
# ПОРОГ НЕ ПОДГОНЯЛСЯ ПОД «РОВНО НОЛЬ РАСХОЖДЕНИЙ»: проверка обязана
# продолжать ловить эти два случая, иначе она перестаёт быть проверкой.
# Допуска, зависящего от инструмента, здесь нет и не будет: относительная мера
# уже решает задачу, а таблица исключений по инструментам создала бы место,
# где ошибку можно спрятать.
TOL_REL = Decimal("5e-4")

# Пол делителя относительной меры: защита от деления на ноль и от вырождения
# относительного отклонения при околонулевых ценах. Он же — ВТОРИЧНОЕ
# абсолютное условие: расхождение засчитывается, только если превышены ОБА
# допуска — относительный TOL_REL и абсолютный EPS_FLOOR.
EPS_FLOOR = Decimal("1e-8")

# Прежний АБСОЛЮТНЫЙ допуск прогона 11.09.2026. Он больше ничего не решает и
# оставлен ровно для одного: по нему считается второе, справочное число
# z0_parity_ohlc_mismatches_abs. Молчаливая замена метрики недопустима —
# сопоставить новый прогон со старым иначе будет нечем.
...
class Deviation:
    ...
    @property
    def relative_floored(self) -> Decimal:
        """Относительное отклонение с ПОЛОМ делителя ``EPS_FLOOR`` (Замер 0.1).

        Отличие от :attr:`relative` ровно одно и оно намеренное: делитель здесь
        никогда не меньше ``EPS_FLOOR``. Это и защита от деления на ноль, и
        защита от вырождения относительной меры при околонулевой цене, где
        относительное отклонение раздувается до бессмысленных величин.

        Именно эта величина печатается как ``z0_parity_rel_dev_max`` и
        ``z0_parity_rel_dev_p50``: распределение обязано оставаться видимым,
        чтобы порог можно было пересмотреть ПО ДАННЫМ, а не заново.
        """
        return self.absolute / max(abs(self.stored), EPS_FLOOR)

    @property
    def is_mismatch(self) -> bool:
        """Расхождение по правилу Замера 0.1: превышены ОБА допуска.

        Абсолютный ``EPS_FLOOR`` оставлен вторичным условием намеренно. Без
        него относительная мера у околонулевой цены засчитывала бы как
        расхождение разницу в последнем знаке хранения ``NUMERIC(20,8)`` —
        то есть шум округления самой базы.
        """
        return self.absolute > EPS_FLOOR and self.relative_floored > TOL_REL

    @property
    def is_mismatch_abs(self) -> bool:
        """Расхождение по ПРЕЖНЕМУ абсолютному правилу 1e-8.

        Ничего не решает. Нужна затем, чтобы прогон Замера 0.1 можно было
        сопоставить с прогоном 11.09.2026 числом, а не рассуждением.
        """
        return self.absolute > OHLC_TOLERANCE
...
def compare_bar(
    ...
    Расхождение считается по правилу Замера 0.1 (:attr:`Deviation.is_mismatch`):
    превышены ОБА допуска — относительный ``TOL_REL`` и абсолютный
    ``EPS_FLOOR``. Число здесь не пишется: оно живёт в одном месте файла.
