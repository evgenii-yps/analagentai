...
ПОЧЕМУ ИМЕННО httpx. Он уже закреплён в requirements и им проект ходит наружу
в остальных местах; новых зависимостей не вводится (§12.1 ТЗ).

ПОДПИСЬ КЛИЕНТА — БРАУЗЕРНАЯ, ИЗ ``src.core.http`` (правка Этапа 8.10.1).
До 28.08.2026 здесь было написано обратное: проверкой из контейнера 22.08.2026
было установлено, что OKX отвечает 200 на штатную подпись ``python-httpx``, и
браузерная не нужна. Это наблюдение было верным тогда и перестало быть верным:
28.08.2026 на сервере тот же публичный эндпоинт ``/api/v5/public/time`` начал
отвечать **403, код 1010** на питоновскую подпись — ДО всякой проверки ключа, —
тогда как ``curl`` получает 200. Прежний текст заменён, а не удалён молча:
расхождение кода с отчётом месячной давности иначе выглядело бы забывчивостью.
...
from src.core.http import exchange_headers
...
def create_http_client() -> httpx.AsyncClient:
    """HTTP-клиент прогона — с браузерной подписью из единого места проекта.

    Заголовки берутся из ``src.core.http.exchange_headers`` и НЕ задаются
    здесь литералом: этим же клиентом пользуются ``scripts/probe_history_depth.py``
    и ``scripts/recon_8_2_0_okx.py``, и подпись у всех троих обязана быть одна.
    Своя подпись в этом файле означала бы, что следующее изменение защиты биржи
    придётся чинить в трёх местах, а забудут — в одном.
    """
