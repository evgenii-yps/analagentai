...
COSTS = Costs(fee_roundtrip_pct=Decimal("0.10"), slippage_pct=Decimal("0.01"))


def test_costs_total() -> None:
    assert COSTS.total == pytest.approx(0.11)


def test_manual_example_buy_up() -> None:
    """Пример 1: buy, цена 100 → 102. Валовая +2%, чистая +1.89%."""
    gross = gross_pnl_pct("buy", 100.0, 102.0)
    assert gross == pytest.approx(2.0)
    assert net_pnl_pct(gross, COSTS) == pytest.approx(1.89)
...
def test_manual_example_sell_up() -> None:
    ...
    assert net_pnl_pct(gross, COSTS) == pytest.approx(-1.11)
...
def test_costs_are_charged_regardless_of_outcome() -> None:
    ...
    assert profit == pytest.approx(4.89)
    assert loss == pytest.approx(-5.11)
...
def test_small_move_is_eaten_by_costs() -> None:
    ...
    4-часовое движение в Этапе 7.1 (0.03%) меньше издержек (0.11%).
    """
