...
1. Поднимите всё окружение одной командой — `.env` не требуется:

   ```bash
   docker compose up --build
   ```

   Стек стартует в ноль ручных шагов на согласованных дефолтах (это же
   обеспечивает автоматический перезапуск на сервере). `.env` необязателен и
   нужен только чтобы переопределить значения — реальный пароль БД или
   Telegram-токен:

   ```bash
   cp .env.example .env   # опционально: отредактируйте под прод
   ```
