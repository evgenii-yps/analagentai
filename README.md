...
│   ├── agent-trade-barrier.cron       # шаблон cron-записи исходов по границам (8.8)
│   ├── agent-trade-baseline.cron      # шаблон cron-записи базовых стратегий (8.9)
...
│   ├── barrier_main.py     # точка входа — исход по границам (Этап 8.8)
│   ├── baseline_main.py    # точка входа — базовые стратегии (Этап 8.9)
...
│   ├── baseline/           # базовые стратегии: линейка сравнения (Этап 8.9)
│   │   ├── strategies.py   # правила шести стратегий, монета (чистое)
│   │   └── runner.py       # суточный расчёт strategy_outcomes
│   ├── barrier/            # исход по границам: вторая оценка (Этап 8.8)
│   │   ├── outcomes.py     # правило «какая граница задета первой» (чистое)
│   │   └── runner.py       # суточный расчёт signal_outcomes_barrier
...
## Вторая оценка исхода: границы (Этап 8.8)

Действующая оценка (`signal_evaluations`, выше) отвечает на вопрос **«где была цена
к сроку»**. Она не отвечает на другой вопрос — **что получил бы человек**: цена могла
дойти до цели и вернуться, могла дойти на час позже замера, могла сначала уйти против
сигнала так глубоко, что реальную позицию закрыли бы с убытком.

Этап 8.8 вводит **вторую, независимую** оценку по правилу «какая граница задета первой».
Обе существуют **параллельно**: ни одна не отменяет другую, и **ни одно решение системы
от новой оценки не зависит** — `LOGIC_VERSION` остаётся 5, текст уведомлений не меняется,
сервисы `decision`, `notify` и `evaluator` не изменялись.

Три уровня от цены решения `P` (замороженной в `signal_targets`):
...
`target_pct` берётся **замороженным** из `signal_targets` — тем, что было названо человеку
в момент сигнала. Подставлять сегодняшнее значение из `risk_targets` запрещено: это
подделка истории. `stop_pct` — параметр `BARRIER_STOP_PCT` (по умолчанию 1.0).

Исходов ровно пять, и перечень закрыт ограничением базы:

| исход | условие |
|---|---|
| `target` | цель задета раньше предела |
| `stop` | предел задет раньше цели |
| `timeout` | до срока не задета ни одна |
| `ambiguous` | обе задеты внутри одного бара — порядок неизвестен |
| `no_data` | ряда свечей на интервале нет |

«Задета» — это **касание внутри свечи** (по `high`/`low`), а не цена закрытия: реальный
ордер срабатывает по касанию. Касание ровно в границу засчитывается. Свеча самого момента
решения в окно не входит — окно строго `t+1 … t+h`.

**Одновременное касание не разрешается догадкой.** Если по инструменту есть минутные свечи
на всём интервале, порядок определяется по ним (`resolution='1m'`). Если минутных нет и обе
границы задеты в одном часовом баре — исход `ambiguous`, и он **никогда** не приводится к
`target` или `stop` эвристикой. Доля таких случаев выводится отдельным числом; выше
`BARRIER_AMBIGUOUS_MAX_PCT` (15%) в журнал идёт отдельная строка
`barrier_metric_unusable=1` — метрика в таком виде малопригодна.

Итог в деньгах `net_pnl_pct` — за вычетом круговых издержек `RISK_COST_ROUNDTRIP_PCT`:
`target` → `+target_pct − cost`, `stop` → `−stop_pct − cost`, `timeout` → фактическое
движение к сроку со знаком по направлению минус `cost`. У `ambiguous` и `no_data`
результата нет (`NULL`): неизвестно, что произошло, — значит, неизвестно и сколько
получил бы человек.

Расчёт живёт в профиле `tools` и в горячий путь не попадает:

```bash
# разовый запуск (cron 04:10 UTC — после целей в 03:40, до выгрузки в 06:20)
docker compose --profile tools run --rm --no-deps barrier

# принудительный пересчёт: снести исходы текущей версии логики и посчитать заново
docker compose --profile tools run --rm --no-deps barrier python -m src.barrier_main --recompute
```

Без флага расчёт **идемпотентен**: повторный запуск на тех же данных не меняет ни одной
строки, и снимок таблицы совпадает побайтно. `--recompute` требуется явно, потому что
пересчёт старых сигналов может **понизить** разрешение: минутные свечи удаляются политикой
хранения через `RETENTION_1M_DAYS` суток, и то, что однажды измерено по минутам, во второй
раз посчитается по часам.

Результат — таблица `signal_outcomes_barrier` (миграция `db/migrations/015_barrier_outcomes.sql`,
откат — `..._rollback.sql`; таблица внесена в `PROTECTED_TABLES` политики хранения). Кроме
исхода в ней хранятся `mae_pct` (максимальное отклонение против сигнала за окно) и `mfe_pct`
(в пользу) — оба **по всему окну**, а не до момента касания: именно поэтому по ним можно
ответить на вопрос «а что было бы при другом уровне предела».

Два запроса анализа (только чтение):

```bash
# §8: уровни предела 0.3 … 3.0 — доля срабатываний и доля напрасных
docker compose exec -T postgres psql -U agenttrade -d agenttrade \
    -f /dev/stdin < analysis/sql/07_stop_levels.sql

# §9: две оценки рядом по каждому сигналу
docker compose exec -T postgres psql -U agenttrade -d agenttrade \
    -f /dev/stdin < analysis/sql/08_metric_compare.sql
```

Предпроверка данных перед расчётом (глубина минутных свечей, покрытие окон,
сигналы без замороженных целей) — `scripts/precheck_8_8.sql`.
Проверка развёртывания — `deploy/verify_8_8.sh`.

Настройки (с дефолтами): `BARRIER_STOP_PCT` (1.0), `BARRIER_AMBIGUOUS_MAX_PCT` (15),
`BARRIER_FINE_TIMEFRAME` (1m), `BARRIER_COARSE_TIMEFRAME` (1h). Издержки берутся из
`RISK_COST_ROUNDTRIP_PCT` — отдельного ключа нет намеренно: две комиссии в одной системе
неминуемо разъехались бы.

> **Значение 1.0% — предположение владельца, а не измерение.** Раздел §8 ТЗ существует
> ровно для того, чтобы это число однажды пересмотрели по данным. Менять его по итогам
> первой же таблицы ТЗ запрещает: сначала доклад, потом решение.

## Линейка сравнения: базовые стратегии (Этап 8.9)

Этап 8.8 дал числа системы. Но сравнивать их не с чем — а без сравнения нельзя
отличить **умение от погоды**. Продажи выглядят прибыльными, только двое суток
рынок падал, и на падающем рынке выигрывает любой сигнал на продажу, включая
брошенную монету.

Этот этап **ничего не улучшает — он измеряет**. Те же моменты, те же цены входа,
та же метрика, тот же период; вместо решений агентов — заведомо безмозглые
правила. `LOGIC_VERSION` остаётся 5, решения системы не меняются.

| Стратегия | Правило | Момент входа |
|---|---|---|
| `always_buy` | покупка всегда | момент сигнала системы |
| `always_sell` | продажа всегда | момент сигнала системы |
| `coin_flip` | направление монетой | момент сигнала системы |
| `system` | фактическое решение системы | момент сигнала системы |
| `grid_buy` | покупка каждый час | ровно в 00 минут, **независимо от системы** |
| `grid_sell` | продажа каждый час | ровно в 00 минут, **независимо от системы** |

Первые четыре отвечают на вопрос «в тех же обстоятельствах безмозглое правило
справилось бы хуже?». Сетка отвечает на **другой** вопрос — «каков вообще фон
рынка?» — и нужна потому, что система выдаёт сигналы неравномерно (например,
чаще при высокой волатильности), а сравнение только по её моментам этого
перекоса не видит.

**Монета детерминирована.** Направление — функция от `(BASELINE_SEED, signal_id,
horizon_h)`, а не результат `random`. Та же тройка всегда даёт то же
направление: на любой машине, в любом порядке обхода, в любом прогоне.
Случайность, которую нельзя повторить, для сравнения непригодна.

**Цель встречного направления берётся за дату входа.** В `signal_targets`
заморожено только направление выданного сигнала — встречной цели там нет вовсе.
Она берётся из `risk_targets` строкой с `computed_at <= момента входа`, и
колонка `target_source` записывает, откуда именно: `frozen` или
`risk_targets:<дата>`. Без этой колонки через месяц нельзя было бы доказать, что
цели не подменялись сегодняшними.

Предел убытка и издержки — **те же** `BARRIER_STOP_PCT` и
`RISK_COST_ROUNDTRIP_PCT`, что у системы; своих у линейки нет намеренно.
Правило исхода тоже не дублируется: `src/baseline` вызывает
`src/barrier/outcomes.py` как есть — вторая реализация рано или поздно разошлась
бы с первой на краевом случае, и сравнение стало бы недействительным незаметно.

Расчёт живёт в **том же** сервисе `barrier` профиля `tools` (нового не заводится),
cron 04:25 UTC — после исходов системы в 04:10:

```bash
# полный расчёт всех шести стратегий
docker compose --profile tools run --rm --no-deps barrier python -m src.baseline_main

# одна стратегия, часть выборки — для замера времени перед полным прогоном
docker compose --profile tools run --rm --no-deps barrier \
    python -m src.baseline_main --strategy coin_flip --since 2026-08-25 --limit 500
```

Без флагов расчёт **идемпотентен**: повторный запуск не меняет ни одной строки
и не читает ни одного окна свечей заново. Результат — таблица `strategy_outcomes`
(миграция `db/migrations/016_strategy_outcomes.sql`, в `PROTECTED_TABLES`).

### Сравнение — и почему одной таблицы средних мало

```bash
# описательные числа: пары, доля target, средний результат
docker compose exec -T postgres psql -U agenttrade -d agenttrade \
    -f /dev/stdin < analysis/sql/09_baseline_compare.sql

# доверительный интервал разницы — читать ОБЯЗАТЕЛЬНО вместе с таблицей
docker compose --profile tools run --rm --no-deps barrier \
    python -m scripts.baseline_bootstrap
```

Разница средних всегда чем-нибудь да отличается от нуля — на выборке в двое
суток даже тогда, когда никакой разницы нет. Поэтому главный вывод даёт не
разница, а **интервал**, в котором она правдоподобно лежит: 10 000 пересборок
выборки с возвращением, интервал 95%, парный бутстрэп на общих созревших парах.
Ответ — одна из трёх формулировок:

- «система лучше, интервал не пересекает ноль»
- «система хуже, интервал не пересекает ноль»
- «различить нельзя, выборки не хватает»

> **Третья формулировка на нынешнем объёме ожидаема, и она не приговор.** Она не
> означает, что система плоха или хороша — только что наблюдений пока мало.
> Заменять её числом без интервала нельзя: это создаёт уверенность, которой у
> данных нет, и решения начинают приниматься по шуму.

Стратегии `grid_buy` и `grid_sell` в попарное сравнение **не входят**: общих пар
с системой у них нет по построению. Их числа стоят рядом (раздел 9.6 запроса) —
как справка о фоне рынка, а не как вычитаемое.

Проверка развёртывания — `deploy/verify_8_9.sh`. Предпроверка данных —
`scripts/precheck_8_9.sql`.

Настройки: `BASELINE_SEED` (20260826), `BASELINE_GRID_TIMEFRAME` (1m).
