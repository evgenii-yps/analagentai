 ...
 *
 * ЭТАП 9.2 §6.1. СТОЛБЕЦ ВРЕМЕНИ БОЛЬШЕ НЕ ПЕРЕПОЛНЯЕТСЯ НА СУТКАХ. Позиция 71
 * (BTC, вход 06.09.2026 08:07, выход 07.09.2026 08:08) прожила 24:00:37, а лист
 * показал 0:00:37: значение верное, а числовой формат ячейки — часовой
 * (`h:mm:ss`), и часы в нём считаются по циферблату, по модулю суток. Пока
 * позиция жила не дольше суток, переполниться формат мог ровно на границе, и
 * такая строка вышла одна из 88. С версии 6 сделки живут 24–48 часов регулярно,
 * и столбец врал бы на большинстве строк, оставаясь правдоподобным.
 *
 * Приёмник теперь оборачивает часовой маркер в квадратные скобки
 * (`h:mm:ss` → `[h]:mm:ss`) у ячеек, где стоит формат ДЛИТЕЛЬНОСТИ, — при
 * создании строки и при дозаписи закрытия. Формат даты не трогается, формула не
 * трогается, значение не трогается: меняется только то, КАК число показано, и
 * только там, где показано оно неверно. Разбор — в :func:`elapsedTimeFormat`.
 *
...
function tableAppend(body) {
  ...
  var warning = pullFormulasDown(sheet, startRow, padded.length, formulaFrom,
                                 noteColumn);
  // ФОРМАТ ДЛИТЕЛЬНОСТИ ЧИНИТСЯ ПОСЛЕ ПРОТЯЖКИ, А НЕ ДО (Этап 9.2 §6.1).
  // Протяжка копирует формулы в новые строки, а формат новая строка получает
  // от строки выше при вставке — то есть переполняющийся формат приезжает
  // вместе с ней. Чинить его надо тогда, когда он уже приехал.
  var formatsFixed = fixElapsedFormats(sheet, startRow, padded.length,
                                       formulaFrom, noteColumn);
  ...
  var answer = { ok: true, inserted: padded.length, updated: 0,
                 ...
                 version: RECEIVER_VERSION };
  if (formatsFixed > 0) answer.formatsFixed = formatsFixed;
...
function tableUpdate(body) {
  ...
  for (var i = 0; i < updates.length; i += 1) {
    ...
    // ФОРМАТ ДЛИТЕЛЬНОСТИ ЧИНИТСЯ И ЗДЕСЬ (Этап 9.2 §6.1), и это не дубль. В
    // строку открытия длительности ещё нет — считать её не из чего, пока нет
    // выхода; она появляется ровно сейчас, вместе с дозаписью столбцов H..J.
    // Починка при создании строки бережёт будущие сделки, починка здесь —
    // сделки, уже лежащие в листе, включая те, что были созданы старым
    // приёмником.
    formatsFixed += fixElapsedFormats(sheet, row, 1,
                                      item.formulaFromColumn || 11, noteColumn);
...
/**
 * ФОРМАТ ДЛИТЕЛЬНОСТИ, ПЕРЕПОЛНЯЮЩИЙСЯ НА СУТКАХ → ФОРМАТ БЕЗ ПЕРЕПОЛНЕНИЯ
 * (Этап 9.2 §6.1). Возвращает исправленный формат либо '' — «править нечего».
 *
 * НАХОДКА ВЛАДЕЛЬЦА 07.09.2026. Позиция 71 (BTC, вход 06.09.2026 08:07, выход
 * 07.09.2026 08:08, исход «истёк срок») прожила 24:00:37, а столбец времени в
 * листе показал 0:00:37. Ошибки в расчёте нет: значение верное, а ЧИСЛОВОЙ
 * ФОРМАТ ячейки — часовой (`h:mm:ss`), и часы в нём считаются ПО ЦИФЕРБЛАТУ,
 * то есть по модулю суток. Ровно двадцать четыре часа выглядят как ноль.
 *
 * ПОЧЕМУ ЭТО ПЕРЕСТАЁТ БЫТЬ МЕЛОЧЬЮ ИМЕННО СЕЙЧАС. До Этапа 9.2 позиция жила
 * не дольше суток, и переполниться формат мог ровно в одной точке — на самой
 * границе; такая строка вышла одна из 88. С версии 6 сделки живут 24–48 часов
 * РЕГУЛЯРНО, и столбец врал бы на большинстве строк, оставаясь при этом
 * правдоподобным: 25 часов показались бы как час.
 *
 * ЧТО ИМЕННО ДЕЛАЕТСЯ. Часовой маркер оборачивается в квадратные скобки:
 * `h:mm:ss` → `[h]:mm:ss`. В Google Таблицах (как и в Excel) скобки означают
 * «не переполнять» — часы копятся сверх суток. Всё остальное в формате —
 * минуты, секунды, доли, текст — не трогается ни символом.
 *
 * ЧЕГО ЗДЕСЬ НЕ ДЕЛАЕТСЯ, И ЭТО ВАЖНЕЕ. Не подменяется формула, не меняется
 * значение, не назначается формат ячейке, у которой формата длительности нет.
 * Формат ДАТЫ (в нём есть d, m, y вне минутной позиции) не трогается вовсе:
 * обернуть часы в скобки у столбца с датой значило бы сломать столбец, который
 * работал. Уже исправленный формат (`[h]`) возвращает '' — второй раз ничего
 * не переписывается.
 */
function elapsedTimeFormat(format) {
  var text = String(format === null || format === undefined ? '' : format);
  if (text.length === 0) return '';
  // Уже без переполнения — править нечего.
  if (text.indexOf('[h]') >= 0 || text.indexOf('[H]') >= 0) return '';
  // Формат должен быть длительностью: часы И минуты И секунды подряд.
  // Одни часы без минут — это не длительность, а, скорее всего, чей-то
  // самодельный формат, и угадывать за владельца нельзя.
  if (!/h+:mm(:ss)?/i.test(text)) return '';
  // Дата внутри формата: d/m/y вне маркеров времени. Обёртка часов в скобки
  // у столбца с датой сломала бы столбец, который работает.
  if (/[dyDY]/.test(text)) return '';
  // Оборачивается ПЕРВАЯ группа часов — та, что стоит перед минутами.
  return text.replace(/(h+)(?=:mm)/i, '[$1]');
}

/**
 * Починить формат длительности в строках листа (Этап 9.2 §6.1).
 *
 * Просматриваются столбцы ``fromColumn``..последний, кроме столбца заметок;
 * ячейке с переполняющимся форматом длительности ставится формат без
 * переполнения. Возвращает число исправленных ячеек — по нему видно в ответе,
 * что правка вообще сработала.
 *
 * ЭТО ЕДИНСТВЕННОЕ МЕСТО, ГДЕ ПРИЁМНИК МЕНЯЕТ ОФОРМЛЕНИЕ ЧУЖОГО ЛИСТА, и
 * оговорить это надо прямо. Правило «формулы владельца не переписываются»
 * остаётся в силе: формула не трогается, значение не трогается, меняется
 * только то, КАК число показано, и только там, где показано оно неверно.
 * Смысл формата при этом сохраняется полностью — «часы:минуты:секунды»
 * остаются часами, минутами и секундами.
 */
function fixElapsedFormats(sheet, startRow, count, fromColumn, noteColumn) {
  var lastCol = sheet.getLastColumn();
  if (lastCol < fromColumn) return 0;
  var fixed = 0;
  for (var row = startRow; row < startRow + count; row += 1) {
    for (var col = fromColumn; col <= lastCol; col += 1) {
      if (col === noteColumn) continue;
      var cell = sheet.getRange(row, col, 1, 1);
      var wanted = elapsedTimeFormat(cell.getNumberFormat());
      if (wanted.length === 0) continue;
      cell.setNumberFormat(wanted);
      fixed += 1;
    }
  }
  return fixed;
