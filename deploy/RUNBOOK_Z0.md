...
```
✅ в итоговом блоке: `z0_unclosed_rows` = **0**, `контрольный опыт §7 (сдвиг окна)` = **не 0** расхождений, `контрольный опыт §8 (незакрытый)` = **1**, `контрольный опыт допуска (§3 ТЗ)` = **все три примера воспроизвелись**, `peak_rss_mb` заметно меньше 1024 (ключи выровнены пробелами — ищите по имени ключа, не по точной строке) — ❌ `код возврата 3` — незакрытые бары в хранилище; ❌ `4` — контрольный опыт не упал, проверка слепа; ❌ `5` — выборка пуста.

Поимённый список расхождений печатается **блоком B** отчёта и сохраняется
копией в `reports/zamer_0_2_parity_<дата>.md` — выпишите его целиком, а не
только число. Поглощённые допуском отклонения видны числом в блоке A; чтобы
увидеть их поимённо, добавьте к команде `--verbose`.

**О `z0_parity_ohlc_mismatches` и коде возврата 2.** Правило Замера 0.2
(допуск — единица последнего значащего знака **грубой** стороны) обязано дать
на боевых данных **7** расхождений: 5 на рубеже 2019→2020 (BTC и ETH,
недельный и месячный бары — помечены `BOUNDARY_ANOMALY`) и 2 дневных за
18.12.2022. Код возврата **2** здесь — правильный результат, а не отказ:
проверка обязана их видеть. ❌ Другое число разбирается **по фактическому
списку** — допуск не подкручивается ни при каких условиях. ❌ Ноль расхождений
при живом контрольном опыте означает, что правило перестало ловить настоящее
рассогласование.
...
а также `z0_parity_ohlc_absorbed` и `z0_parity_ohlc_absorbed_precision` — по
ним видно, сколько отклонений объясняется точностью записи.
...
| шаг 14 | `z0_parity_*` (включая `absorbed`, `absorbed_precision`, `boundary_anomaly`, `real`), **поимённый список блока B**, `z0_unclosed_rows`, `peak_rss_mb`, предложенный порог по объёму |
