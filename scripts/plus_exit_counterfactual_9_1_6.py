...
PRICE_STORAGE_PLACES = 8  # positions.exit_price — NUMERIC(20,8)
...
def breakeven_price(
    entry_price: float, cost_pct: float, *, side: str, gross: bool
) -> float:
    """ЦЕНА БЕЗУБЫТКА §3.6 ТЗ: цена, при которой итог сделки равен нулю.
    ...
    направления лежит по другую сторону цены входа, и молча посчитать продажу
    по формуле покупки было бы нельзя.

    ``gross=True`` — «плюс» ВАЛОВОЙ, ``Pbe = P0`` (вариант D §4 ТЗ). Издержки
    при этом никуда не деваются: итог такой сделки равен ровно минус издержкам,
    и вариант D нужен ровно затем, чтобы показать, сколько сделок разделяет эта
    разница.
    """
    if entry_price <= 0:
