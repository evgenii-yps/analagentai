...
async def stream_sample() -> tuple[PairMatrix, Composition, dict[str, int], int]:
    ...
    while True:
        ...
        #
        # ГРАНИЦА ОБЯЗАНА ДВИГАТЬСЯ ВПЕРЁД, И ЭТО ПРОВЕРЯЕТСЯ. Если чтение
        # перестанет учитывать ключ-границу, каждая порция вернёт одно и то же
        # начало таблицы, и цикл будет крутиться ВЕЧНО — печатая ноль строк и
        # выглядя работающим. Это тот же класс дефекта, что тихий обрыв: снаружи
        # неотличимо от долгого счёта. Обнаружив, что граница не сдвинулась,
        # расчёт падает сразу и называет причину.
        if current is None or (after is not None and current <= after):
            raise RuntimeError(
                "граница чтения не сдвинулась вперёд "
                f"({after} -> {current}) — порции возвращают одно и то же; "
                "проверьте условие ключа-границы в запросе"
            )
        after = current
