...
PROTECTED_TABLES: frozenset[str] = frozenset(
    {
        ...
        "strategy_outcomes",
        # Этап 8.10 §6: исходы подвижного выхода, тринадцать вариантов на пару.
        # Причина та же и она сильнее: пересчёт возможен только пока живы
        # минутные свечи, а на часовых подвижный выход не измеряется вовсе —
        # вершина внутри часового бара неизвестна. Потеряв таблицу, ответ на
        # вопрос владельца «подвижный выход лучше фиксированного?» придётся
        # ждать заново столько же суток, сколько копились свечи.
        "trailing_outcomes",
    }
)
...
RETENTION_RULES: list[tuple[str, int, str]] = [
    ...
    # ЖУРНАЛ ОТКАЗОВ ВО ВХОДЕ: 90 суток (§7.1 ТЗ 9.3). Срок и причина названы
    # в ТЗ числом: при ~7200 решениях в сутки и отказе почти на каждом таблица
    # растёт примерно на 2,5 млн строк в год. Бюджет места это выдерживает, но
    # прополка обязательна — и она стоит здесь, а не отдельной задачей: второе
    # расписание для той же работы однажды разошлось бы с первым.
    #
    # ТАБЛИЦА МОЖЕТ ОТСУТСТВОВАТЬ (миграция 028 не применена), и правило это
    # переживает: _delete_in_batches зовёт psql, чья ошибка не роняет задачу —
    # тот же путь, что у любой другой не созданной ещё таблицы.
    _rule("position_rejections", "RETENTION_POSITION_REJECTIONS_DAYS", "90"),
]
...
# СТОЛБЕЦ ВРЕМЕНИ, ЕСЛИ ОН НЕ ``ts``. Почти во всех таблицах проекта отметка
# времени называется ``ts``, и правило удаления опирается на это. Журнал
# отказов (миграция 028, §7.1 ТЗ 9.3) назвал её ``ts_utc`` — так её называет ТЗ,
# и переименовывать столбец ради удобства прополки нельзя: имя столбца видит
# владелец в запросах проверки, а мапу видит только этот файл.
#
# ОТСУТСТВИЕ ТАБЛИЦЫ В ЭТОЙ МАПЕ ЗНАЧИТ ``ts``, а не «нет правила»: иначе
# добавление таблицы сюда стало бы обязательным шагом, о котором забывают.
TS_COLUMNS: dict[str, str] = {"position_rejections": "ts_utc"}
