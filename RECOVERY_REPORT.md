...
```
  origin/claude/agent-trade-infra-stage-1-7hb1p0   <- СОХРАНЕНА (неслитые коммиты, см. §3)
  origin/claude/explain-signal-script-7ms5p7       <- СОХРАНЕНА (неслитые коммиты, см. §3)
  origin/claude/local-stack-exchange-audit-so6rjz  <- СОХРАНЕНА (неслитые коммиты, см. §3)
...
## 3. Архивные ветки: что удалено, что сохранено

Перед удалением каждая ветка проверена на неслитые коммиты
(`git merge-base --is-ancestor`, `git cherry`, сравнение деревьев). Правило:
**если в ветке есть коммиты/файлы, отсутствующие в `main`, — ветка НЕ удаляется,
её коммиты перечисляются, решение за заказчиком.** Никакой код не потерян.

### Удалены (полностью вошли в `main`, потерь нет)
...
### СОХРАНЕНЫ — есть неслитые коммиты, требуется решение заказчика
...
> `explain_signal.py` и (б) артефакты Этапа 6.4 (аудит бирж). Если эта работа вам
> больше не нужна — напишите, и я удалю все три ветки. Если что-то из неё нужно
> сохранить — я вынесу нужное в `main` отдельным коммитом до удаления. **По
> умолчанию три ветки оставлены нетронутыми, чтобы ничего не потерять.**
...
| Архивные ветки удалены либо для каждой указана причина | ✅ | 3 удалены; 3 сохранены с перечнем неслитых коммитов (§3) |
...
3. **Причина инцидента — пять похожих длинных имён веток `claude/…-xxxxxx`**, для
   каждой GitHub предлагает «Compare & pull request». Обошли: сократили число
   веток (после решения по §3 в репозитории останется только `main`), и — по §9
   ТЗ — впредь слияние в `main` и удаление рабочей ветки выполняет Claude Code в
   конце этапа, а не заказчик через веб-интерфейс.
