...
> Решение об их судьбе отложено — см. «Открытый вопрос» ниже.

### 🔖 ОТКРЫТЫЙ ВОПРОС (решается в следующем этапе Claude Code)

Заказчик 13.08.2026 попросил зафиксировать этот вопрос в отчёте, чтобы его
разобрал следующий сеанс Claude Code, а не решать сейчас. Ниже — всё, что нужно
для решения без повторного расследования.

**Вопрос.** Что сделать с тремя ветками, содержащими уникальную (отсутствующую в
`main`) работу, прежде чем удалять их из `origin`:

1. `claude/explain-signal-script-7ms5p7` — уникально: `scripts/explain_signal.py`,
   `reports/signal_1.md`, `reports/.gitkeep`. Коммиты: `219b837`, `246f0e6`,
   `e8fcfb9`. Это самостоятельный разовый скрипт разбора сигнала (вход/стоп/цель/
   RR) на данных OKX. В production-контур `main` никогда не входил.
2. `claude/local-stack-exchange-audit-so6rjz` — уникально: работа Этапа 6.4 —
   `scripts/exchange_audit.py`, `scripts/init_env.sh`, `scripts/start.sh`,
   `scripts/__init__.py`, `tests/test_exchange_audit.py`,
   `reports/exchange_audit_2026-07-30.md`, `reports/stage_6_4_report.md`.
   Коммиты (без merge-мусора инцидента): `5852d2d`, `9429892`, `7d80d76`.
   Именно эти файлы Этапа 6.4 в ТЗ 6.7.1 §1 названы «ошибочно просочившимися» в
   `main` и удалёнными оттуда.
3. `claude/agent-trade-infra-stage-1-7hb1p0` — старая ветка по умолчанию; в ходе
   инцидента её tip передвинули на `bbcf492`, и в неё «затянуло» те же уникальные
   коммиты Этапа 6.4, что и в п.2. Своей отдельной уникальной работы, кроме
   Этапа 6.4 и merge-мусора, не содержит.

**Варианты решения (для следующего сеанса):**
- **A. Всё в утиль** — работа не нужна; подтвердить, что production-логика
  Этапов 1–6.6 целиком в `main`, и оставить удаление веток заказчику.
- **B. Сохранить `explain_signal`** — вынести `scripts/explain_signal.py`
  (при необходимости с примером отчёта) в `main` отдельным коммитом; артефакты
  Этапа 6.4 — в утиль.
- **C. Сохранить и Этап 6.4, и `explain_signal`** — вынести оба набора в `main`,
  затем ветки можно удалять.

**Ограничение по исполнению.** Программное удаление веток из `origin` в текущей
среде заблокировано GitHub (HTTP 403, см. врезку выше). Поэтому фактическое
удаление любых веток выполняет заказчик вручную (веб-интерфейс → Branches → 🗑),
даже после того как следующий сеанс перенесёт нужную работу в `main`.

**Рекомендация Claude Code.** Вариант **A**: уникальная работа — это отдельный
вспомогательный скрипт и артефакты давно завершённого Этапа 6.4 (аудит бирж уже
сыграл свою роль — стек развёрнут на OKX). Ни то, ни другое не влияет на
production-контур и окно наблюдения. Но окончательный выбор — за заказчиком/
следующим сеансом.
