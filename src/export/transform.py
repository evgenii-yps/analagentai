...
def build_position_close_note_tail(row: dict[str, Any]) -> str:
    ...
    ИМЕННО ХВОСТ, А НЕ ВЕСЬ ТЕКСТ, УХОДИТ В ЛИСТ ПРИ ЗАКРЫТИИ (§16 ТЗ). Столбец
    заметок — единственное место строки, куда человек пишет руками: пока сделка
    шла, владелец мог занести туда своё наблюдение. Прислать заметку целиком и
    заменить ею ячейку значило бы стереть его текст — молча и безвозвратно.
    Приёмник дописывает этот хвост к тому, что в ячейке уже есть.
    ...
    return (
        f" · {words} · итог системы {float(row['net_pnl_pct']):+.2f}% "
...
def build_position_close_note(
    ...
    """ПОЛНЫЙ текст заметки после закрытия: открытие плюс хвост.

    В ЛИСТ ЦЕЛИКОМ ОН УХОДИТ ТОЛЬКО В ОДНОМ СЛУЧАЕ — когда строку открытия не
    нашли и сделка кладётся новой полной строкой: там дописывать не к чему,
    ячейка пуста. В обычном закрытии уходит один лишь хвост
    (:func:`build_position_close_note_tail`), чтобы не стереть текст владельца.

    Метка остаётся ПЕРВОЙ: по ней строку нашли, по ней же найдут снова.
    ...
    """
    return f"{build_position_note(row)}{build_position_close_note_tail(row)}"
