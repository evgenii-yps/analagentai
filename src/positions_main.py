...
async def _main() -> None:
    ...
    # Журнал отказов во входе (миграция 028, §7.1 ТЗ 9.3) — тем же порядком и
    # по той же причине: порядок «сначала миграция, потом образ» соблюсти
    # удаётся не всегда, а служба без этой таблицы обязана вести сделки.
    await db.ensure_position_rejections_schema()
