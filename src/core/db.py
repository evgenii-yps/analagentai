...
class DB:
    ...
    async def get_backtest_candles(
        ...
        """Свечи спота ЗАДАННОГО МАСШТАБА из ``backtest.candles``, по времени.
        ...
        ``bar`` ОБЯЗАТЕЛЕН И ЗНАЧЕНИЯ ПО УМОЛЧАНИЮ ИМЕТЬ НЕ ДОЛЖЕН — никогда,
        даже если однажды покажется, что все вызовы всё равно передают ``1H``.
        С Замера 0 в этой таблице лежат не только часовые бары: рядом с ``1H``
        живут ``1Dutc``, ``1Wutc`` и ``1Mutc``. Запрос без фильтра по масштабу
        вернул бы их вперемешку, и расчёт целей получил бы ряд с дублями меток
        и разорванной непрерывностью — измерено контрольным опытом
        ``scripts/risk_targets_parity_z0.py --control``: 107 дублей, ряд
        признан негодным, цель уехала на 8%. Заголовок «часовые свечи» в этом
        месте стоял до Замера 0 и был заменён: он подсказывал, что масштаб тут
        формальность.

        Единственный источник истины о том, что фильтр на месте, — сам запрос
        ниже; его отсутствие ловит тест
        ``tests/test_zamer_0.py::test_every_candles_query_filters_by_bar``.
        """
    ...
    async def ensure_positions_schema(self) -> None:
        ...
        Проверка «один сигнал — не больше одной позиции» в коде переживает
        ровно до первой гонки: сервис перезапустили, две итерации наложились —
        и позиций стало две. База такого не допустит вовсе.

        ПРАВИЛО «ОДИН ИНСТРУМЕНТ — ОДНА ОТКРЫТАЯ ПОЗИЦИЯ» ДЕЙСТВУЕТ ТОЛЬКО ДО
        ВЕРСИИ 7 (§7.2 ТЗ 7, миграция 027). Условие индекса дополнено
        ``logic_version < 7``: строки версий 5 и 6 остаются под тем же
        запретом, под каким открывались, а версия 7 допускает по одному токену
        сколько угодно одновременных позиций.

        СТАРЫЙ ИНДЕКС СНИМАЕТСЯ ПО ОПРЕДЕЛЕНИЮ, А НЕ ПО ИМЕНИ, и это не
        придирка. Метод зовётся при КАЖДОМ старте службы позиций, а
        ``CREATE UNIQUE INDEX IF NOT EXISTS`` смотрит только на ИМЯ: индекс с
        прежним условием остался бы нетронутым — том, на котором миграцию 027
        не применяли, продолжал бы жить по правилу версии 6 молча, без единой
        ошибки, а объяснять пришлось бы «почему сделок по-прежнему мало».
        """
    ...
    async def get_position_candidates(
        ...
        *,
        ...
        ЗДЕСЬ ПРОВЕРЯЕТСЯ ШЕСТЬ УСЛОВИЙ ИЗ ВОСЬМИ: решение, версия логики,
        ...
        одно условие единственности: по этому сигналу позиция НИКОГДА не
        открывалась.

        УСЛОВИЯ «ПО ИНСТРУМЕНТУ НЕТ ОТКРЫТОЙ ПОЗИЦИИ» ЗДЕСЬ БОЛЬШЕ НЕТ (§3 A2
        ТЗ 7): правило «один инструмент — одна позиция» снято, по одному токену
        допускается произвольное число одновременно открытых позиций. Условие
        «один сигнал — не больше одной позиции» ОСТАЁТСЯ: оно не про частоту
        сделок, а про повторное чтение одного и того же сигнала после
        перезапуска.

        ПАУЗА ПО ТОКЕНУ В ЗАПРОС НЕ ВНОСИТСЯ НАМЕРЕННО (§4 B4 ТЗ 7). Отсеки её
        здесь — и придержанный паузой кандидат не дошёл бы до правила, то есть
        не попал бы ни в журнал, ни в счётчик ``token_pause``; а измерить,
        сколько поток режет пауза, требуется прямо.
