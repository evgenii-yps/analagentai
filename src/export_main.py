...
#
# ЭТАП 9.2 ПОДНЯЛ ТРЕБОВАНИЕ ДО 9.2 (§6.1). Приёмник 9.1.2.2 пишет строки
# верно, но столбец времени в них ПЕРЕПОЛНЯЕТСЯ на сутках: 24:00:37 выглядит
# как 0:00:37. Пока сделки жили меньше суток, это была одна строка из 88; с
...
def _split_ambiguous_open(
    result: sheets.SheetsResult,
    rows: list[dict[str, Any]],
    log: structlog.types.WrappedLogger,
) -> tuple[set[int], set[str]]:
    """Делит отказ ``table_append`` по занятой метке НА ДВА СЛУЧАЯ (§5 ТЗ 9.1.2.3).

    ПОЧЕМУ ДВА СЛУЧАЯ, А НЕ ОДИН. Приёмник отвечает на занятую метку одинаково —
    строки не создал, метку вернул в ``ambiguous``, — но за одним ответом стоят
    два разных положения дел, и они требуют противоположных действий:

      * СТРОК С МЕТКОЙ РОВНО ОДНА. Строка открытия этой позиции в листе УЖЕ
        ЕСТЬ, и она одна: метка уникальна, значит принадлежит именно ей. Терять
        тут нечего — потеряна только ОТМЕТКА В БАЗЕ, а без неё позиция никогда
        не попадёт в очередь дозаписи закрытия (``fetch_positions_pending_close``
        требует ``sheet_opened_at IS NOT NULL``), и выход по сделке не будет
        дописан НИКОГДА. Отметка восстанавливается — база приводится в
        соответствие с листом, лист не трогается вовсе.
      * СТРОК ДВЕ И БОЛЬШЕ. Это защита Этапа 9.1.2.2, и она остаётся дословно:
        какая из строк чья — неизвестно, и любой выбор был бы догадкой. Ни
        отметки, ни новой строки, ``error`` с ``sheets_ambiguous_marker=1``.

    НОЛЬ НОМЕРОВ СТРОК — ТОЖЕ ОТКАЗ, а не «метки нет». Случай «метки в листе
    нет» сюда не приходит вовсе: приёмник тогда создаёт строку обычным порядком
    и в ``ambiguous`` её не возвращает. Пустой список номеров означает другое —
    повтор метки ВНУТРИ одной пачки (``seenInBatch`` приёмника): строки нет ни
    одной, создана она не была, и отмечать нечего.

    МЕТКА, КОТОРОЙ НЕТ В ПАЧКЕ, — тоже отказ. Позиции по такой метке мы не
    знаем, и восстанавливать отметку не у чего; разбирать номер из текста метки
    нельзя по той же причине, что и в :func:`_report_ambiguous`.

    Возвращает ``(id позиций с восстановленной отметкой, метки-отказы)``.
    """
    if not result.ambiguous:
        return (set(), set())
    by_marker = {position_marker(row["id"]): row for row in rows}
    ...
    for item in result.ambiguous:
        ...
        if not marker:
            continue
        sheet_rows: list[int] = []
        for number in item.get("rows") or []:
            try:
                sheet_rows.append(int(number))
            except (TypeError, ValueError):
                continue
        row = by_marker.get(marker)
        if row is not None and len(sheet_rows) == 1:
            recovered.add(int(row["id"]))
            # УРОВЕНЬ info, А НЕ error, И ЭТО НЕ СМЯГЧЕНИЕ ФОРМУЛИРОВКИ. Сделка
            # в листе есть, строка одна, дозапись закрытия в неё теперь пойдёт:
            # чинить человеку нечего. Признак ``sheets_marker_restored=1`` стоит
            # отдельным полем, чтобы восстановленные отметки можно было
            # посчитать по журналу одной командой и не спутать с записанными
            # заново: строка в листе НЕ СОЗДАВАЛАСЬ.
            log.info(
                "Торговый журнал: строка открытия уже есть — отметка восстановлена",
                sheets_marker_restored=1,
...
async def _export_trades(
    ...
    if pending_open:
        ...
        # МЕТКА УЖЕ ЗАНЯТА — ДВА РАЗНЫХ ПОЛОЖЕНИЯ ДЕЛ ЗА ОДНИМ ОТВЕТОМ (§5 ТЗ
        # 9.1.2.3), и различает их :func:`_split_ambiguous_open` по числу
        # возвращённых номеров строк:
        #
        #   * СТРОК ДВЕ И БОЛЬШЕ — отметка НЕ СТАВИТСЯ (§3 ТЗ 9.1.2.2). Отметка
        #     необратима: поставив её здесь, мы объявили бы позицию выгруженной,
        #     тогда как своей строки в листе у неё нет — есть несколько чужих с
        #     её меткой. Позиция останется в очереди и будет выгружаться каждым
        #     следующим прогоном, пока лист не приведут в порядок. Повторяющаяся
        #     ошибка в журнале — цена за то, чтобы сделка не потерялась молча.
        #   * СТРОКА РОВНО ОДНА — отметка ВОССТАНАВЛИВАЕТСЯ. Строка открытия уже
        #     есть и принадлежит этой позиции; расходится с листом не он, а
        #     база. Без отметки позиция не попадёт в очередь дозаписи закрытия
        #     никогда — и выход по сделке не будет дописан никогда.
        recovered, skipped = _split_ambiguous_open(result, pending_open, log)
        opened_ids = [
            int(row["id"]) for row in pending_open
            if position_marker(row["id"]) not in skipped
        ]
        created = len([pid for pid in opened_ids if pid not in recovered])

        # СВЕРКА СО СЧЁТОМ ПРИЁМНИКА — ПЕРЕД ОТМЕТКОЙ, А НЕ ПОСЛЕ. ``created``
        # мы вывели своим счётом, ``inserted`` пришло от того, кто писал в лист;
        # сойтись они обязаны. Не сошлись — значит, ответ разобран не так, как
        # он был составлен, и КАКИЕ ИМЕННО строки созданы, мы не знаем. Отметить
        # в этот момент всю пачку значило бы объявить выгруженной сделку, строки
        # у которой может не быть, — ровно та потеря, ради которой написан этот
        # этап. Поэтому не отмечается НИЧЕГО: созданные строки подберёт
        # следующий прогон — их метки в листе уже заняты ровно одной строкой, и
        # это восстановление отметки, а не второй экземпляр сделки.
        if result.inserted != created:
            log.error(
                "Торговый журнал: свой счёт созданных строк разошёлся с ответом "
                "приёмника — отметки не поставлены",
                sheets_created_mismatch=1,
                created=created, inserted=result.inserted,
                recovered=len(recovered), skipped_ambiguous=len(skipped),
                batch=len(pending_open),
            )
            raise ExportError(
                f"торговый журнал, открытия: приёмник создал {result.inserted} "
                f"строк, а по ответу их насчитано {created} — отметки не "
                "поставлены, пачка повторится на следующем запуске"
            )
        ...
        log.info(
            ...
            # ВОССТАНОВЛЕННЫЕ ОТМЕТКИ СЧИТАЮТСЯ ОТДЕЛЬНО ОТ СОЗДАННЫХ СТРОК:
            # строка в листе при восстановлении не появляется, и сложить их в
            # одно число значило бы отчитаться о записи, которой не было.
        ...
        updates = [
            {
                ...
                # Откуда начинаются столбцы формул владельца. Нужен приёмнику
                # для починки формата длительности (§6.1 ТЗ 9.2): именно в этой
                # дозаписи у строки впервые появляется время выхода, а значит и
                # длительность сделки, которую столбец времени показывает.
                "formulaFromColumn": POSITION_FORMULA_FROM_COLUMN,
            }
