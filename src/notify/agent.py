...
# --- УВЕДОМЛЕНИЯ О СИГНАЛАХ: ПОСЛЕДНЯЯ ВЕРСИЯ, ПРИ КОТОРОЙ ОНИ УХОДЯТ ---------
#
# §5 C1 ТЗ 7: с версии логики 7 в Telegram уходят ТОЛЬКО события сделок —
# открытие, закрытие и суточная сводка. Сообщения о сигналах прекращаются
# полностью.
#
# ГРАНИЦА ЗАПИСАНА ВЕРСИЕЙ ЛОГИКИ, А НЕ ОТДЕЛЬНЫМ ВЫКЛЮЧАТЕЛЕМ, и это выбор, а
# не экономия на настройке. Во-первых, откат (§10 ТЗ) состоит из возврата .env и
# LOGIC_VERSION=6 — и сигнальные уведомления обязаны вернуться вместе с ним, не
# требуя правки кода. Во-вторых, выключатель в .env однажды переставили бы, не
# заметив, что вместе с сигналами в поток вернулись сорок сообщений в час, от
# которых этап и уходит.
#
# ЧТО ПРИ ЭТОМ НЕ МЕНЯЕТСЯ ВОВСЕ (§5 C3 ТЗ). Сигналы собираются, оцениваются и
# пишутся в базу ровно как прежде: этот модуль не пишет в ``signals`` ни одной
# содержательной строки — он только помечает, обработан ли сигнал очередью
# уведомлений. Порог NOTIFY_MIN_PROBABILITY, NOTIFY_MIN_AGENTS, выдержка,
...
class NotifyAgent:
    ...
    async def process_once(self) -> None:
        ...
        # УВЕДОМЛЕНИЯ О СИГНАЛАХ ПРЕКРАЩЕНЫ (§5 C1 ТЗ 7) — но очередь
        # обрабатывается ПО-ПРЕЖНЕМУ. Просто перестать её читать было бы хуже
        # молчания: неотправленные сигналы копились бы в выборке вечно, и
        # каждая итерация перечитывала бы их заново. Каждый сигнал поэтому
        # «поглощается» ровно так же, как поглощается сегодня сигнал, никому не
        # ушедший: ``notified = TRUE``, ``notified_at`` НЕ ставится — отправки
        # не было, и утверждать обратное нельзя.
        #
