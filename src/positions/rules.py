...
# ПАУЗА ПО ТОКЕНУ (§5 ТЗ 9.3, ``cooldown`` в словаре ТЗ). ЕДИНСТВЕННЫЙ
# ограничитель, который в бою СРАБАТЫВАЕТ: ограничители ЧИСЛА (``max_open``,
# ``instrument_busy``, ``no_free_capital``) при боевых настройках недостижимы,
# но живы и включаются одной строкой в ``.env``.
REASON_TOKEN_PAUSE = "token_pause"
# ПОТОЛОК ЧИСЛА ОДНОВРЕМЕННО ОТКРЫТЫХ ПОЗИЦИЙ — ВЕРНУЛСЯ В КОД (§3, §4.4
# ТЗ 9.3), и это прямая отмена решения этапа 7, который вырезал и проверку, и
# причину. Довод этапа 7 («ветка, которая никогда не срабатывает, однажды
# снова начнёт срабатывать от чужой правки») верен, но цена его оказалась
# выше: пока проверки нет вовсе, возврат ограничения — это откат ветки и
# пересборка образов, а не правка одной строки в ``.env`` и перезапуск. ТЗ 9.3
# выбирает второе явно: поток может оказаться неуправляемым, и тормоз обязан
# быть под рукой.
#
# ОГРАЖДЕНИЕ ОТ ТОГО САМОГО РИСКА — НЕ КОММЕНТАРИЙ, А ТЕСТ: при
# ``max_open = 0`` причина недостижима, и это проверяется опытом §10.1
# (20 сигналов → 20 позиций, отказов ``max_open`` ноль) вместе с его
# контролем (``POSITION_MAX_OPEN=5`` → ровно 5 и 15 отказов).
REASON_MAX_OPEN = "max_open"
# ИНВАРИАНТ «ОДИН ИНСТРУМЕНТ — ОДНА ОТКРЫТАЯ ПОЗИЦИЯ» — тоже вернулся, и тоже
# выключателем, а не удалением (§4.1, §10.4 ТЗ 9.3). При
# ``POSITION_ONE_PER_TOKEN=false`` недостижим; ``true`` возвращает прежнее
# правило и роняет контроль опыта §10.4 — ради чего он и нужен.
...
# СООТВЕТСТВИЕ СЛОВАРЮ ТЗ 9.3 §4.4. ТЗ перечисляет восемь машиночитаемых
# причин; в коде их одиннадцать, и это не расхождение, а разрешение §3 ТЗ
# («исполнитель использует существующие имена») плюс два случая, которые ТЗ
# писалось без кода в руках и потому не различает:
#
#   ТЗ                       код
#   'not_buy'                not_buy
#   'low_probability'        low_probability
#   'no_quorum'              degraded            (признак ставит decision)
...
#   —                        no_frozen_target
#   —                        instrument_busy     (§4.1: выключатель инварианта)
#
# ДВА ПОСЛЕДНИХ СЛУЧАЯ РАЗВОРАЧИВАТЬ ОБРАТНО НЕЛЬЗЯ. «Сигнал устарел» и «нет
# свежей свечи» — разные события: первое о том, что система молчала, второе о
# том, что молчал коллектор; сложив их в одно 'stale_candle', мы потеряли бы
# именно ту разницу, по которой чинят. «Цель не заморожена» ТЗ не называет
# вовсе, а причина существует с этапа 8.2 и срабатывает.
...
def should_open(
    *,
    ...
    open_count: int = 0,
    max_open: int = 0,
    token_open_count: int = 0,
    one_per_token: bool = False,
) -> OpenDecision:
    """Условия допуска к сделке, все одновременно. Первое несработавшее — отказ.

    ЧТО ДЕЛАЕТ ЭТАП 9.3 (§3, §4, §5 ТЗ). Система ограничивает не ЧИСЛО
    одновременных сделок, а их ЧАСТОТУ. Ограничители числа при этом НЕ УДАЛЕНЫ
    ИЗ КОДА — они обойдены нулевым значением настройки:

      * ПОТОЛОК ОТКРЫТЫХ ПОЗИЦИЙ (``max_open``) — проверка на месте,
        ``max_open <= 0`` означает «ограничение не применяется», а НЕ «ноль
        позиций». Это самая дорогая ошибка, какую здесь можно сделать: при
        прочтении «ноль» отказ получил бы КАЖДЫЙ кандидат, журнал показывал бы
        честную причину, и объяснение искали бы в рынке. Поэтому у неё свой
        опыт (§10.1) и своя намеренная поломка (§11.1).
      * ИНВАРИАНТ «ОДИН ИНСТРУМЕНТ — ОДНА ОТКРЫТАЯ ПОЗИЦИЯ»
        (``instrument_busy``) — проверка на месте и включается настройкой
        ``one_per_token``; при ``False`` по одному токену допускается сколько
        угодно одновременных позиций.
      * БЮДЖЕТ (``no_free_capital``) — проверка на месте; при
        ``POSITION_BUDGET_USD=0`` свободных денег бесконечно много.

      И ЕДИНСТВЕННЫЙ ОГРАНИЧИТЕЛЬ, КОТОРЫЙ В БОЮ СРАБАТЫВАЕТ, — ПАУЗА ПО
      ТОКЕНУ (``token_pause``): новая позиция по токену не открывается, если по
      нему уже открывались менее ``token_pause_sec`` назад.

    ПОЧЕМУ ОГРАНИЧИТЕЛИ ЧИСЛА ВЕРНУЛИСЬ В КОД (§3 ТЗ 9.3, «запрет на удаление
    кода»). Этап 7 вырезал их целиком, доводом «ветка, которая никогда не
    срабатывает, однажды снова начнёт срабатывать от чужой правки». Довод
    верен, и ограждением от него служат опыты §10.1 и §10.4 вместе с их
    контролями. Но цена вырезания выше: возврат ограничения превращается из
    правки одной строки в ``.env`` в откат ветки и пересборку образов — а
    поток сделок при снятых слотах может оказаться неуправляемым, и тормоз
    обязан быть под рукой у владельца, а не у исполнителя.

    ПОЧЕМУ ЧИСЛО ОТКРЫТЫХ ПОЗИЦИЙ ПРИХОДИТ ПАРАМЕТРОМ, А НЕ ЧИТАЕТСЯ ЗДЕСЬ.
    Модуль чистый: ни базы, ни «сейчас». То же касается ``token_open_count`` —
    сколько позиций по ЭТОМУ токену открыто прямо сейчас.
    ...
    в Telegram. Настройки уведомлений (NOTIFY_HOLD_MIN, NOTIFY_MAX_PER_HOUR,
    NOTIFY_COOLDOWN_SEC) к сделкам не применялись никогда и не применяются: они
    берегут внимание человека, а не моделируют торговлю.
    ...
    (§12.2 ТЗ: предсказано не меньше 70% таких отказов) оказалась бы завышена
    не по делу.
    """
    ...
    # ИНВАРИАНТ «ОДИН ИНСТРУМЕНТ — ОДНА ПОЗИЦИЯ» — ТОЛЬКО ПРИ ВКЛЮЧЁННОЙ
    # НАСТРОЙКЕ (§4.1 ТЗ 9.3). При ``one_per_token=False`` ветка недостижима, и
    # счётчик ``instrument_busy`` обязан показывать ноль.
    if one_per_token and int(token_open_count) > 0:
        return OpenDecision(False, REASON_INSTRUMENT_BUSY)
    # ПОТОЛОК ОТКРЫТЫХ ПОЗИЦИЙ. НОЛЬ — ЭТО «ОГРАНИЧЕНИЕ НЕ ПРИМЕНЯЕТСЯ», А НЕ
    # «НОЛЬ ПОЗИЦИЙ» (§3 ТЗ 9.3). Условие ``max_open > 0`` стоит ПЕРВЫМ
    # намеренно: написанное вторым (``open_count >= max_open and max_open > 0``)
    # оно читалось бы так же, но любая правка, потерявшая вторую половину,
    # отвергала бы каждого кандидата.
    if int(max_open) > 0 and int(open_count) >= int(max_open):
        return OpenDecision(False, REASON_MAX_OPEN)
