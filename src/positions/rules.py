...
последней известной цене, чтобы она не осталась НЕИЗМЕРЕННОЙ навсегда. В статистику
точности и прибыльности такие закрытия не идут: выдуманная цена не должна
входить в результат системы наравне с измеренными.
...
# известной цене, чтобы не висеть открытой вечно. В статистику точности и
...
# ПАУЗА ПО ТОКЕНУ (§4 ТЗ 7). Единственный ограничитель ЧАСТОТЫ сделок, пришедший
# на смену двум ограничителям ЧИСЛА: слотам (``slots_full``) и правилу «один
# инструмент — одна позиция» (``instrument_busy``). Обе прежние причины удалены
# вместе с их проверками, а не оставлены «на всякий случай»: ветка, которая
# никогда не срабатывает, однажды снова начнёт срабатывать от чужой правки, и
# объяснить, почему сделок стало вдвое меньше, будет нечем.
REASON_TOKEN_PAUSE = "token_pause"
...
#
# С ВЕРСИИ 7 ЭТА ПРИЧИНА НЕДОСТИЖИМА, И ТАК ЗАДУМАНО (§3 A3 ТЗ 7). Бюджет
# переведён в режим «без ограничения» (``POSITION_BUDGET_USD=0``), свободных
# денег бесконечно много, и счётчик этой причины обязан показывать ноль.
# Проверка и счётчик СОХРАНЕНЫ намеренно — в отличие от ``slots_full``, который
# удалён: разница между «денег нет» и «слотов нет» станет содержательной в тот
# день, когда бюджет снова сделают конечным, и обнаружить её тогда будет
# некому, если ограждение снять сейчас.
...
def token_pause_left_sec(
    last_open_age_sec: float | None, token_pause_sec: float
) -> float:
    """Сколько секунд паузы по токену ещё не истекло. ЧИСТАЯ функция (§4 ТЗ 7).

    ``last_open_age_sec`` — сколько секунд прошло с ОТКРЫТИЯ последней позиции
    по этому токену; ``None`` означает «позиций по токену не открывалось вовсе».

    ОТСЧЁТ ОТ ОТКРЫТИЯ, А НЕ ОТ ЗАКРЫТИЯ (§4 B3 ТЗ), и это не деталь
    реализации. Отсчёт от закрытия сделал бы паузу зависимой от ДЛИТЕЛЬНОСТИ
    сделки — а у версий 6 и 7 предела убытка нет, и позиция живёт до 48 часов.
    Правило «час после закрытия» блокировало бы токен почти на двое суток, то
    есть вернуло бы ровно то ограничение, которое этап снимает.

    НОЛЬ ВОЗВРАЩАЕТСЯ И ПРИ ВЫКЛЮЧЕННОЙ ПАУЗЕ (``token_pause_sec <= 0``, §4 B5),
    и при истёкшей: оба случая означают «ничто не держит», и различать их
    вызывающему незачем. Отрицательного остатка не бывает — «пауза кончилась
    два часа назад» и «пауза кончилась только что» одинаково разрешают вход.
    """
    if token_pause_sec <= 0 or last_open_age_sec is None:
        return 0.0
    return max(0.0, float(token_pause_sec) - float(last_open_age_sec))
...
def should_open(
    *,
    ...
    last_open_age_sec: float | None,
    token_pause_sec: float,
    ...
    """Девять условий, все одновременно. Первый несработавший — и есть отказ.

    ЧТО ИЗМЕНИЛОСЬ В ВЕРСИИ 7 (§3, §4 ТЗ 7). Система перестала ограничивать
    ЧИСЛО одновременных сделок и начала ограничивать их ЧАСТОТУ. Отсюда минус
    два условия и плюс одно:

      * УШЛО «один инструмент — одна позиция» (``instrument_busy``). По одному
        токену допускается произвольное число одновременно открытых позиций.
      * УШЁЛ потолок числа открытых позиций (``slots_full``). Проверки нет
        вовсе — не «есть, но всегда проходит»: ветка, которая никогда не
        срабатывает, однажды снова начнёт срабатывать от чужой правки.
      * ПРИШЛА ПАУЗА ПО ТОКЕНУ (``token_pause``): новая позиция по токену не
        открывается, если по нему уже открывались менее ``token_pause_sec``
        назад.

    ПОЧЕМУ СНЯЛИ ОБА ОГРАНИЧИТЕЛЯ, А НЕ ОДИН. Правило «один инструмент — одна
    позиция» было ВАЖНЕЕ слотов: предела убытка у версии 6 нет, позиция живёт
    до 48 часов, и токен оставался бы занят почти двое суток. Сними одни слоты
    — и число сделок не выросло бы, а объяснение «мы же сняли ограничение»
    выглядело бы убедительно.

    ПРОВЕРКА ДЕНЕГ ОСТАЛАСЬ НА СВОЁМ МЕСТЕ и стала недостижимой (§3 A3 ТЗ 7):
    при ``POSITION_BUDGET_USD=0`` свободных денег бесконечно много. Счётчик
    ``no_free_capital`` обязан показывать ноль — это проверяемое утверждение,
    а не догадка.
    ...
    в Telegram, — и с версии 7 сигналы в Telegram не уходят вовсе (§5 ТЗ 7),
    так что смотреть стало не на что. Настройки уведомлений (NOTIFY_HOLD_MIN,
    NOTIFY_MAX_PER_HOUR, NOTIFY_COOLDOWN_SEC) к сделкам не применялись никогда
    и не применяются: они берегут внимание человека, а не моделируют торговлю.

    ПОРЯДОК ПРОВЕРОК СОДЕРЖАТЕЛЕН. Свойства самого сигнала идут раньше свойств
    окружения: причина «сигнал не подходил» точнее причины «идёт пауза по
    токену», когда верны обе. Иначе журнал показывал бы паузу там, где на самом
    деле не было ни одного годного сигнала, и доля отказов ``token_pause``
    (§8 ТЗ: предсказано больше половины) оказалась бы завышена не по делу.
    """
    ...
    # Остаток паузы СЧИТАЕТСЯ ТОЙ ЖЕ функцией, которой сервис печатает его в
    # журнал: два выражения для одного числа однажды разошлись бы, и в журнале
    # стояло бы «до конца паузы 4 минуты» у отказа, случившегося по другой
    # причине.
    if token_pause_left_sec(last_open_age_sec, token_pause_sec) > 0:
        return OpenDecision(False, REASON_TOKEN_PAUSE)
...
def check_gap_exit(
    *,
    ...
    инструменту, позиция осталась бы открытой НАВСЕГДА — и до версии 7 заняла
    бы навсегда ещё и слот.
