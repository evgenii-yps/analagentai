...
ПРИМЕНЯЮТСЯ И НЕ СТАВЯТСЯ (§5 C5 ТЗ 7). До версии 7 их было не больше десяти в
сутки по построению — пять слотов, открытие и закрытие; слотов больше нет, и
сообщений может стать тридцать пар в сутки. Это ровно то, что владелец хочет
увидеть: станет невыносимо — решение примут по факту, а не по предположению.

С ВЕРСИИ 7 ЭТО ЕДИНСТВЕННЫЕ СООБЩЕНИЯ СИСТЕМЫ ЧЕЛОВЕКУ, не считая суточной
сводки: уведомления о сигналах прекращены (§5 C1 ТЗ 7).
"""
