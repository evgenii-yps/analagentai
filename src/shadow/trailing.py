...
def closing_moment(exit_bar_ts: datetime, resolution: str) -> datetime:
    """Момент ЗАКРЫТИЯ позиции по бару выхода — то, что лежит в ``closed_at``.

    ПОЧЕМУ ЭТО НЕ ОДНО И ТО ЖЕ С ``exit_bar_ts``. Правило выхода
    (``rules.check_exit``) возвращает время ОТКРЫТИЯ бара: точнее ряд свечей не
    знает. А ``src/positions/runner.py`` записывает в ``closed_at`` время его
    ЗАКРЫТИЯ — ``decision.exit_bar_ts + 60 секунд``, — потому что бар прожит
    целиком и относить выход к его открытию значило бы закрывать позицию раньше
    события, по которому она закрыта.

    ПЕРВАЯ РЕДАКЦИЯ СВЕРЯЛА ОДНО С ДРУГИМ НАПРЯМУЮ, и на боевой базе 31.08.2026
    все одиннадцать позиций разошлись ровно на 60 секунд. Это была не ошибка
    расчёта, а сравнение разных величин: допуск оставался нулевым, но сверялись
    открытие бара с его закрытием.

    ПРИВЕДЕНИЕ К ОДНОЙ ВЕЛИЧИНЕ — НЕ ПОБЛАЖКА. Допуск по-прежнему нулевой;
    меняется только то, ЧТО с чем сверяется. После правки расхождений обязано
    быть ноль без единой уступки; если их не ноль — расхождение настоящее.

    ДЛИНА БАРА БЕРЁТСЯ ПО РАЗРЕШЕНИЮ, а не литералом 60. У ``runner`` там литерал,
    и он верен: ``positions_resolution_chk`` допускает только ``1m``. Повторить
    здесь тот же литерал значило бы завести второе место, знающее длину бара, и
    оно разошлось бы с первым в тот день, когда позиции научатся жить на часовом
    ряде. Что правило ``runner`` не изменилось, проверяется отдельной проверкой
    по тексту его исходника.

    У ЗАКРЫТИЙ ПО ПРОБЕЛУ (``data_gap``) момент другой — ``now``, — и вывести его
    из бара нельзя. Такие позиции в замер не входят вовсе (§3.1 ТЗ), поэтому
    этой функции они не касаются.
    """
    return exit_bar_ts + bar_step(resolution)
...
def armed_scan(
    ...
    *,
    ...
    ``until`` — БАР ВЫХОДА ВАРИАНТА ВКЛЮЧИТЕЛЬНО, дальше которого смотреть
    нельзя. Первая редакция просматривала ВСЁ окно, и это была ошибка по
    существу, а не мелочь: вариант, вышедший по пределу на первом баре, получал
    ``armed = true`` от бара, случившегося через полчаса ПОСЛЕ его выхода.
    Механизм такую сделку не трогал — её уже не было, — а ЧИСЛО 1 отвечает
    ровно на вопрос «сколько сделок механизм задел». Ошибка завышала главное
    число этапа и на боевой базе 31.08.2026 уронила расчёт позиции 6.

    ``None`` означает «вариант дожил до конца окна» (``timeout``, ``no_data``)
    либо «бара выхода не существует» (``ambiguous``: порядок событий внутри бара
    неизвестен). В обоих случаях просмотр всего отрезка ТОЧЕН, а не приблизителен:
    у ``ambiguous`` первым порога достигает как раз тот бар, на котором вариант
    и остановился — достигни его бар раньше, правило включило бы подвижный выход
    и до неоднозначности дело не дошло бы.
    ...
    for bar in prefix:
        if until is not None and bar.ts > until:
            break
        reached = (
...
def _check_invariants(outcome: ShadowOutcome) -> None:
    ...
    ВТОРОЕ УТВЕРЖДЕНИЕ СУЖЕНО ДО «МЕЖДУ БАРАМИ», и вот его точная граница.
    Предел не может сработать ПОСЛЕ включения, случившегося на ПРЕДЫДУЩЕМ баре:
    уровень отката при положительной вершине лежит выше цены входа, а предел
    ниже, и цена, дошедшая до предела, обязана была пройти уровень отката
    раньше. ВНУТРИ ОДНОГО БАРА это неверно — максимум бара может дойти до порога
    включения, а минимум того же бара до предела, и порядок событий внутри
    минуты ряду свечей неизвестен. Такой случай правило 8.10 разрешает не
    догадкой, а исходом ``ambiguous``, и своего способа здесь не изобретается:
    выход ``stop`` до этого случая просто не доходит.

    Поэтому утверждение и остаётся строгим: после сужения области задетости до
    жизни самого варианта (:func:`armed_scan`, ``until``) выход ``stop`` при
    ``armed = true`` невозможен ни между барами, ни внутри бара.
    """
    ...
    if outcome.exit_reason == trailing_rule.EXIT_STOP and outcome.armed:
        raise AssertionError(
            f"{outcome.variant}: выход stop при armed=true МЕЖДУ БАРАМИ — "
            "после включения подвижного выхода на предыдущем баре предел "
            "сработать не может; одновременное касание в ОДНОМ баре даёт "
            "ambiguous, а не stop"
        )
    ...
    *,
    ...
    for item in outcomes:
        armed, armed_at = armed_scan(
            ...
            # Дальше бара выхода механизм этой сделки уже не касался.
            until=item.hit_at,
        )
